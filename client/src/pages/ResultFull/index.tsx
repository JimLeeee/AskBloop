...
const ResultFull = ({
  ...
  const handleCustExplain = useCallback(
    (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
      e.stopPropagation();
      if (!result) {
        return;
      ...
      setSelectedLines([0, endLine]);
      setRightPanelOpen(false);
      let custquery='使用中文详细解释代码的作用';
      setSubmittedQuery(
        `#custexplain=${custquery}|${result.relativePath}:0-${endLine}-${Date.now()}`,
      );
      setChatOpen(true);
    },
    [result?.code, result?.relativePath],
        ...
        <div className="w-full flex flex-col overflow-auto flex-1">
          ...
          >
            ...
            <div className="flex-shrink-0 flex items-center gap-2">
              ...
              <Button size="tiny" onClick={handleCustExplain}>
                <Sparkles raw sizeClassName="w-3.5 h-3.5" />
                <Trans>中文Explain</Trans>
              </Button>
              <FileMenu
