...
const Chat = () => {
  ...
  const makeSearch = useCallback(
    (
      query: string,
      options?: { filePath: string; lineStart: string; lineEnd: string; customquery: string; },
    ) => {
      if (!query) {
        return;
      }
      console.log('pathtest:',navigatedItem)
      ...
      //         options.filePath,
      //       )}&line_start=${options.lineStart}&line_end=${options.lineEnd}&userquery=${encodeURIComponent(query)}`
      ...
      const url = `${apiUrl}/answer${
        options
          ...
          : `/explain?relative_path=${encodeURIComponent(
            navigatedItem?.path || '',
          )}&line_start=${selectedLines?.[0]}&line_end=${selectedLines?.[1]}&userquery=${encodeURIComponent(query)}`
      }&repo_ref=${tab.repoRef}${
        threadId
          ? `&thread_id=${threadId}${
              queryIdToEdit ? `&parent_query_id=${queryIdToEdit}` : ''
  ...
  useEffect(() => {
    ...
    //自定义按钮，自定义查询分析
    // let customquery = '统计一下文件里有哪些函数,各自的作用,需要包含函数的链接，最后对调用关系总结一下'
  if (submittedQuery.startsWith('#explain_')) {
      ...
      userQuery = t(
        `{{query}} the file {{filePath}}, from lines {{lineStart}} - {{lineEnd}}`,
        // `use Chinese to answer. Explain the purpose of the file {{filePath}}, from lines {{lineStart}} - {{lineEnd}}`,
        {
          query: 'Explain the purpose of',
    ...
    if (submittedQuery.startsWith('#custexplain=')) {
      const [prefix, ending] = submittedQuery.split(':');
      const [lineStart, lineEnd] = ending.split('-');
      const [customquery,filePath] = prefix.slice(13).split('|');
      options = {
        filePath,
        lineStart,
        lineEnd,
      ...
      userQuery = t(
        `answer in chinese. {{query}} the file is {{filePath}}, from lines {{lineStart}} - {{lineEnd}}`,
        {
          query: customquery,
          lineStart: Number(lineStart) + 1,
          lineEnd: Number(lineEnd) + 1,
  ...
  const onSubmit = useCallback(
    (e?: FormEvent) => {
      ...
      blurInput();
      setSubmittedQuery(
        `#custexplain=${inputValue}|${navigatedItem?.path}:0-${selectedLines?.[1]}-${Date.now()}`,
      );
      // setSubmittedQuery(
